...
export default function ReturnDetail({ params }: PageProps) {
  ...
  const fetchReturnDetail = async () => {
    try {
      setLoading(true);
      const res = await OrderService.getReturnById(returnId);
  ...
  const openConfirmModal = (type: ActionType) => {
    const config = {
      APPROVE: {
        ...
        description:
          "Hệ thống sẽ ghi nhận bạn đồng ý nhận lại sản phẩm hoàn trả này. Khách hàng sẽ nhận được thông báo để gửi hàng về.",
      },
      REJECT: {
        title: "Từ chối yêu cầu trả hàng?",
        description:
          "Hành động này không thể hoàn tác. Bạn có chắc chắn muốn từ chối tiếp nhận yêu cầu hoàn trả này từ người mua?",
      },
      COMPLETE: {
        title: "Xác nhận hoàn tiền?",
        description:
          "Hãy chắc chắn rằng bạn đã kiểm tra hàng hoàn đầy đủ và không bị hư hỏng. Hành động này sẽ hoàn tất yêu cầu và hoàn tiền cho người mua.",
      },
    };
  ...
  const totalRefundAmount =
    returnDetail?.itemReturns?.reduce(
      (sum: number, i: any) => sum + (i.refundAmount || 0),
      0,
    ) || 0;
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-orange-50 text-center p-4">
        ...
        <p className="text-sm text-gray-500 mt-1 max-w-sm">
          Mã đơn trả hàng không tồn tại hoặc bạn không có quyền truy cập vào dữ
          liệu này.
        </p>
        <Link
          href="/vendor/returns"
          className="mt-5 text-sm font-semibold text-orange-600 bg-white border border-orange-200 px-4 py-2 rounded-lg shadow-sm hover:bg-orange-50 transition"
        >
          Quay lại danh sách
        </Link>
      </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card className="border border-orange-200 p-6 bg-white rounded-xl shadow-sm space-y-5">
                <div className="flex justify-between items-start border-b border-gray-100 pb-4">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-semibold tracking-wide border ${statuses[currentStatusMapped as keyof typeof statuses] || "bg-gray-100 text-gray-700"}`}
                  >
                    {currentStatusMapped}
                  </span>
                </div>
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 p-3 rounded-lg border border-gray-100">
                    ...
                    <div>
                      ...
                      <span className="text-gray-900 font-medium">
                        {new Date(returnDetail.createdAt).toLocaleString(
                          "vi-VN",
                        )}
                      </span>
                    </div>
                ...
                <div className="bg-orange-50/60 rounded-xl p-4 border border-orange-100">
                  <span className="text-xs font-bold uppercase text-orange-800 tracking-wider block mb-1.5">
                    Lý do hoàn trả từ người mua:
                  </span>
              ...
              <Card className="border border-orange-200 p-6 bg-white rounded-xl shadow-sm">
                <h3 className="font-bold text-sm text-gray-900 mb-4 border-b border-gray-100 pb-3">
                  Danh sách sản phẩm hệ thống nhận hoàn trả
                </h3>
                <div className="divide-y divide-gray-100">
                  {returnDetail.itemReturns?.map((item: any) => (
                    <div
                      key={item.itemReturnId}
                      className="py-4 first:pt-0 last:pb-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
                    >
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="font-semibold text-sm text-gray-900">
                            Mã sản phẩm:{" "}
                            <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200 text-gray-700 ml-1">
                              {item.productId}
                            </span>
                          </p>
                        </div>
                        <p className="text-xs text-gray-400 pl-4">
                          ID dòng đơn hàng: {item.orderDetailId}
                        </p>
                        <p className="text-xs text-gray-600 pl-4 font-medium">
                          Số lượng trả hàng:{" "}
                      ...
                      <div className="text-left md:text-right pl-4 md:pl-0">
                        ...
                        <span className="font-extrabold text-orange-600 text-base">
                          {(item.refundAmount || 0).toLocaleString("vi-VN")}đ
                        </span>
                      </div>
                    </div>
                  ))}
                ...
                <div className="space-y-3.5 text-sm">
                  <div className="flex justify-between text-gray-500">
                    ...
                    <span className="font-semibold text-gray-900">
                      {returnDetail.itemReturns?.length || 0} mục
                    </span>
                  </div>
                  <div className="flex justify-between text-gray-500">
                    <span>Cập nhật tiến độ cuối:</span>
                    <span className="font-semibold text-gray-900">
                      {new Date(returnDetail.updatedAt).toLocaleDateString(
                        "vi-VN",
                      )}
                    </span>
                  </div>
              ...
              {returnDetail.status === "COMPLETED" && (
                <div className="bg-emerald-50 text-emerald-900 border border-emerald-200 rounded-xl p-4 text-xs space-y-1.5 shadow-sm">
                  <p className="font-bold text-emerald-800 flex items-center gap-1.5 text-sm">
                    <span className="flex items-center justify-center w-4 h-4 bg-emerald-600 text-white rounded-full text-[10px]">
                      ✓
                    </span>
                    Quy trình hoàn tất thành công
                  </p>
                  <p className="text-emerald-700 leading-relaxed pl-5">
                    Đơn hoàn trả đã đóng lại. Người mua đã nhận đủ tiền hoàn và
                    hệ thống đã ghi nhận khấu trừ doanh số dòng tiền của ví
                    doanh nghiệp.
                  </p>
                </div>
              )}
